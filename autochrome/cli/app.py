...
from autochrome.api.data import Project, RenderElement, EngineError
from autochrome.api.engine import Engine
...
def render(
    ...
    if not project_path or not os.path.isfile(project_path):
        raise EngineError(f'project path not valid: {project_path}')
    try:
        data = storage.read_data(project_path)
    except ValueError as e:
        logger.debug(e)
        raise EngineError(f'project is not valid: {project_path}') from None
    ...
    if animation_path:
        try:
            ...
            logger.debug(e)
            raise EngineError(f'animation is not valid: {animation_path}') from None
    ...
    try:
        ...
        raise EngineError('failed to start engine') from e
    ...
    for i, frame in enumerate(range(frame_start, frame_end)):
        ...
        if not result:
            raise EngineError('an error occurred while rendering')
