...
)
{
    ...
    write_imagef(output, p, value);
    // rgba = (float4) (value, 0, 0, 0);
    // write_imagef(output, p, rgba);
}

__kernel void spectral_grain(
    __constant float* spectral,
    write_only image2d_t output,
    __constant float2* lambda_lut,
    const int samples,
    const float grain_sigma,
    ...
    for (int i = 0; i < samples; i++)
    {
        ...
        for(uint cell_x = bounds.x; cell_x <= bounds.z; cell_x++) // x-cell number
        {
            ...
            for(uint cell_y = bounds.y; cell_y <= bounds.w; cell_y++) // y-cell number
            {
                ...
                // for(uint wavelength_index = 0; wavelength_index <= 31; wavelength_index++)
                // {
                ...
                //     if (c == 0){
                //         // float2 s = = gaussian;
                //         // int2 uv convert_int2(max(floor(s), 0.0f));

                //         u = read_imagef(image, sampler, uv).x;
                //     } else if (c == 1) {
                //         // float2 sgaussian;
                //         // s.x += 4.0f * rand_gaussian(&p_monte_carlo) / bounds_scale.x;
                //         // s.y += 4.0f * rand_gaussian(&p_monte_carlo) / bounds_scale.y;
                //         // int2 uv =  convert_int2(max(floor(s), 0.0f));
                //         u = read_imagef(image, sampler, uv).y;
                //     } else if (c == 2) {
                //         // float2 s  = gaussian;
                //         // s.x += 0.5f * rand_gaussian(&p_monte_carlo) / bounds_scale.x;
                //         // s.y += 0.5f * rand_gaussian(&p_monte_carlo) / bounds_scale.y;
                //         // int2 uv == convert_int2(max(floor(s), 0.0f));
                //         u = read_imagef(image, sampler, uv).z;
                //     }
